...
PRIV_KEY = "6c5fdbf4e19403fbaf955475406a378f2f554a35d54018daa30b9b964e9f709b"
PUB_KEY = "8658075430a6ed7d0061dcc7aef5a5540a58006980e681ae301d42f065f9e39e99585d6d10c6153deda4fd8d31a75bc322c59e6edb707c07f41a1b2832eeb286"
...
def DecodeBuffer(_buffer, _offset, _outbuffer):
    ...
    if not ret[0]:
        ...
        if -1 == fixpos:
            ...
            _outbuffer.extend("[F]decode_log_util.py decode error len=%d, result:%s \n" % (fixpos, ret[1]))
            _offset += fixpos
    ...
    if seq != 0 and seq != 1 and lastseq != 0 and seq != (lastseq + 1):
        _outbuffer.extend("[F]decode_log_util.py log seq:%d-%d is missing\n" % (lastseq + 1, seq - 1))
    ...
    try:
        ...
        traceback.print_exc()
        _outbuffer.extend("[F]decode_log_util.py decompress err, " + str(e) + "\n")
